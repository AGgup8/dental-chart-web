...
function ChartModel(dataStructure) {
    ...
    function loadChart() {
        if (localStorage.chart !== null) {
            chartModel.setChart(JSON.parse(localStorage.chart))
                .then((result) => { $.publish("updated") });
        }
    };
    ...
    function updateChart(tooth, surface, data) {
        chartModel.updateData(tooth, surface, data)
            .then((result) => {
                $.publish("updated");
                saveChart();
            });
...
function ChartView(model) {
    ...
    function compareToActiveSurface(surface) {
        if (selectedSurface == $(surface).data("surface") && selectedTooth == $(surface).data("tooth")) {
            return true;
        } else {
            return false;
        }
    ...
    function setUpClickListeners() {
        ...
        $(".dropdown").on('click', '.dropdown-arrow', function () {
            ...
            if ((selectedTooth == ($(this).parent().data("tooth")) && (selectedSurface) == "whole")) {
                resetSelectedTooth();
            } else {
        ...
        $(".dropdown").on('click', 'p', function () {
            ...
            $.publish("update", { "tooth": selectedTooth, "surface": selectedSurface, "newData": $(this).data("code") });
        ...
        $(".cavity-dropdown-list p").click(function () {
            ...
            $.publish("update", { "tooth": selectedTooth, "surface": selectedSurface, "newData": newData });
        ...
        $('.extra').each(function () {
            ...
            const surface = "extra";
            $.publish("update", { "tooth": `${tooth}`, "surface": surface, "newData": newData });
        }).on("focus", function () {
    ...
    function render() {
        ...
        for (const toothNumber in modelData) {
            ...
            const tooth = $("#tooth-chart").find(`.tooth[data-tooth='${toothNumber}']`);
            ...
            if (tooth.length) {
                ...
                for (const code in modelData[toothNumber]) {
                    ...
                    const surface = tooth.find((`[data-tooth='${toothNumber}'][data-surface='${code}']`));

                    if ((code == "occlusal") || (code == "top") || (code == "bottom") || (code == "left") || (code == "right")) {
                        ...
                        switch (modelData[toothNumber][code]) {
                            case "M":
                                img = createImage("images/missing-line.svg", "tooth-center", "missing tooth image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "FS":
                                img = createImage("images/fissure-sealant.svg", "tooth-center", "fissure sealant image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "IMP":
                                img = createImage("images/implant.svg", "tooth-center", "implant image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "PE":
                                img = createImage("images/partially-erupted.svg", "tooth-center", "partialy erupted image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "BA":
                                img = createImage("images/bridge-abutment.svg", "tooth-center", "bridge abutment image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "BP":
                                img = createImage("images/bridge-pontic.svg", "tooth-center", "bridge pontic image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "PRR":
                                img = createImage("images/preventative-resin-restoration.svg", "tooth-center", "PRR image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "FGC":
                                img = createImage("images/full-gold-crown.svg", "tooth-center", "full gold crown image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "RE":
                                img = createImage("images/recently-extracted.svg", "tooth-center", "recently extracted image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "F":
                                img = createImage("images/fracture.svg", "tooth-center", "fracture image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "PBC":
                                img = createImage("images/porcelain-bonded-crown.svg", "tooth-center", "porcelain bonded crown image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "PV":
                                img = createImage("images/porcelain-veneer.svg", "tooth-center", "porcelain veneer image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "CR":
                                img = createImage("images/crown.svg", "tooth-center", "crown image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "A":
                                img = createImage("images/artificial-tooth.svg", "tooth-center", "artificial tooth image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "FMC":
                                img = createImage("images/full-metal-crown.svg", "tooth-center", "full metal crown image");
                                $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                break;
                            case "IN":
                                if (toothNumber < 29) {
                                    img = createImage("images/standing-arrow-down.svg", "tooth-center", "tooth instanding image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                } else {
                                    img = createImage("images/standing-arrow-up.svg", "tooth-center", "tooth instanding image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                }
                                break;
                            case "OUT":
                                if (toothNumber < 29) {
                                    img = createImage("images/standing-arrow-up.svg", "tooth-center", "tooth outstanding image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                } else {
                                    img = createImage("images/standing-arrow-down.svg", "tooth-center", "tooth outstanding image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                }
                                break;
                            case "RM":
                                if ((toothNumber > 10 && toothNumber < 19) || (toothNumber > 40 && toothNumber < 49)) {
                                    img = createImage("images/rotated-right-arrow.svg", "tooth-center", "tooth rotated mesially image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                } else {
                                    img = createImage("images/rotated-left-arrow.svg", "tooth-center", "tooth rotated mesially image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                }
                                break;
                            case "RD":
                                if ((toothNumber > 10 && toothNumber < 19) || (toothNumber > 40 && toothNumber < 49)) {
                                    img = createImage("images/rotated-left-arrow.svg", "tooth-center", "tooth rotated distally image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                } else {
                                    img = createImage("images/rotated-right-arrow.svg", "tooth-center", "tooth rotated distally image");
                                    $(`.symbol-container[data-tooth='${toothNumber}']`).append(img);
                                }
                                break;
                            default:
                        ...
                        const extraText = $(`[data-tooth='${toothNumber}']`).children("textarea");
